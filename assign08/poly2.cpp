...
void Poly::insertTerm(Term t)
{
   float tempe;
   float tempc;
   for(int i =0; i < numTerms; i++)//loop through array positions
   {
      if(terms[i].getExpon() == t.getExpon())//if the exponents are the same
      {
//         t.setExpon(terms[i].getExpon + t.getExpon);// t object exponent
         //   t.setCoeff(terms[i].getCoeff() + t.getCoeff());// t object coefficents add together      
         tempc = terms[i].getCoeff() + t.getCoeff();// t object coefficents add together`
         t.setCoeff(tempc);// t object coefficents add together       
         if(tempc != 0)// if the t coefficent dont add up to zero
         {
            terms[i] = t;//the t object gets assigned to that array position
         }
         else if(tempc == 0)//if the t coefficent adds up to zero
         {
            terms[i].setCoeff(0);//set the coefficent in that array position to zero
            // terms[i].setExpon(0); 
         }
      }
      else//if not these conditions
         terms[i] = t;// add it to the array
   }      
}
